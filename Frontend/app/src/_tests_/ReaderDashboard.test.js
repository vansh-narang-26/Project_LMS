...
const mockLocalStorage = (() => {
    let store = {};
    return {
        getItem: jest.fn(key => store[key] || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Imt5QGdtYWlsLmNvbSIsImlkIjozMywicm9sZSI6InJlYWRlciJ9.duAruSzkrdO1Y57CySLYQDpi9yuIFp2C_5YAWBsBFpo"),
...
describe("ReaderDashboard Component", () => {
    ...
    //     await waitFor(() => {
    //         expect(screen.findByRole('paragraph',{name:'Author 111'})).toBeInTheDocument();
    //         // expect(screen.getByText("Author 111")).toBeInTheDocument();
    //         // expect(screen.getByText("Version 111")).toBeInTheDocument();
    ...
    //     // Verify API was called correctly
    //     expect(axios.get).toHaveBeenCalledWith(
    //         "http://localhost:8000/api/reader/raise-request/",
    //         {
    //             headers: {
    //                 "Content-Type": "application/json",
    //                 "Authorization": "Bearer mock-token"
